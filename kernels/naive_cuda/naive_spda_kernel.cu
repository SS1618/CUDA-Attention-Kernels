...
__global__ void naive_spda_kernel(const float* __restrict__ Q, const float* __restrict__ K, const float* __restrict__ V, float* __restrict__ output, float* __restrict__ S_matrix, int batch_size, int seq_len, int num_heads, int head_dim) {
...
at::Tensor naive_spda_cuda(at::Tensor Q, at::Tensor K, at::Tensor V) {
